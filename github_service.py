...
Features:
  ...
  - Target Repository: https://github.com/hongquang699/Qrcode
"""
import os
import sys
import subprocess
...
DEFAULT_REPO_URL = "https://github.com/hongquang699/Qrcode.git"
...
def git_push(commit_message: str = None) -> dict:
    ...
    try:
        ...
        # 4. git commit
        result = subprocess.run(
            ["git", "-c", "safe.directory=*", "-c", "user.name=QR-Code Backup", "-c", "user.email=backup@qrcode.local", "commit", "-m", commit_message],
            cwd=PROJECT_ROOT,
            capture_output=True, text=True, encoding="utf-8", timeout=30
        )
